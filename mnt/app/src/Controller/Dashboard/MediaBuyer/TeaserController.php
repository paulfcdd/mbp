...
class TeaserController extends DashboardController
{
    ...
    {
        $teasers = $this->entityManager->getRepository(Teaser::class)->getMediaBuyerTeasersList($this->getUser());
    ...
    public function listAjaxAction(Request $request, DashboardExtension $twigExtensions)
    {
        ...
        foreach ($teasers as $teasersItem) {
            $statistic = $teasersItem->getStatistic();
            $json[] = [
                ...
                $this->convertToUserCurrency($statistic->getECPM(), $this->getUser()),
