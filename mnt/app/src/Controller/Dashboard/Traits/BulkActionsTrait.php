...
trait BulkActionsTrait
{
        ...
        try {
            ...
            foreach($itemsList as $className => $idList){
                ...
                if(stripos($className, 'sub') !== false){
                    $this->addFlash('success', $this->getFlashMessage('teaser_sub_groups_save_and_redirect', [$idString, $action]));
                } else {
    ...
    private function changeBuyerNewsRotation(News $news, $status)
    {
        ...
        $mediabuyerNewsRotation = $this->entityManager->getRepository(MediabuyerNewsRotation::class)->findOneBy([
            'news' => $news,
            'mediabuyer' => $this->getUser()
        ]);
