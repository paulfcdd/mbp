...
trait NewsFinanceTrait
{
    ...
    public function getDataJson(array $data)
    {
        ...
        foreach($data as $dataItem) {
            $dataJson[] = [
                ...
                round($this->calculateProfitReal($dataItem['revenue_real'], $dataItem['costs'])),
                round($dataItem['profit_forecasted']),
                round($this->calculateROIReal($dataItem['revenue_real'], $dataItem['costs'])),
    ...
    private function calculateProfitReal($revenueReal, $cost) {
        return $revenueReal - $cost;
    }

    private function calculateROIReal($revenueReal, $cost) {
        if ($cost != 0 ) {
            return ($revenueReal / $cost) * 100 - 100;
        }
    ...
    private function buildConversionsCountSql($sources, $news, $mediaBuyer, $request, $statusName)
    {
        ...
        $sql = $this->addNewsAndSourcesArrsToSql($sql);
    ...
    public function getNewsFinanceCount(User $mediaBuyer)
    {
    ...
    private function addNewsAndSourcesArrsToSql($sql) {
        if (!empty($sources)) {
