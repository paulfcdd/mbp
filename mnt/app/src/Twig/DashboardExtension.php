...
use App\Command\TeaserStatisticCommand;
...
class DashboardExtension extends AppExtension
{
    public function getFunctions()
    {
        return [
            ...
            new TwigFunction('render_teasers_counter_labels', [$this, 'renderTeasersCounterLabels']),
            ...
            new TwigFunction('show_teaser_statistic_gathering_date', [$this, 'showTeaserStatisticsGatheringDate']),
    ...
    public function showTeaserStatisticsGatheringDate()
    {
        /** @var CronDate $lastStatisticGatheringDate */
        $lastStatisticGatheringDate = $this->entityManager->getRepository(CronDate::class)->findOneBy([
            'slug' => TeaserStatisticCommand::SLUG
        ], ['id'=>'DESC'], 1, 0);

        return $this->twigEnvironment->render('dashboard/partials/table/teaser_statistics_gathering_info.html.twig', [
            'lastStatisticGatheringDate' => $lastStatisticGatheringDate
        ]);
    }
    ...
    {
        return $this->currencyConverter->convertToUserCurrency($price, $user);
    }
     ...
     * @throws \Twig\Error\SyntaxError
     */
    public function renderBulkActionSelector(string $deletePath = null, string $setActivePath = null, string $setDisablePath = null, string $changeTeaserSubGroup = null)
    {
        $pathArray = [];
    ...
    public function renderCounterLabels(User $user)
    {
        ...
        switch ($userRole) {
            case 'ROLE_ADMIN':
                ...
                $parameters = $this->countNewsByType($news);
                break;
            ...
            case 'ROLE_JOURNALIST':
                $news = $this->entityManager->getRepository(News::class)->getJournalistNewsList($user);
                $parameters = $this->countNewsByType($news);
                break;
            default:
                $parameters = [];
        ...
        return $this->twigEnvironment->render('dashboard/partials/news_counter_labels.html.twig', [
            'parameters' => $parameters
        ]);
    }

    public function renderTeasersCounterLabels(User $user)
    {
        $teasers = $this->entityManager->getRepository(Teaser::class)->findBy(['user' => $user]);
        $parameters = $this->countTeasersByType($teasers);
        return $this->twigEnvironment->render('dashboard/partials/teasers_counter_labels.html.twig', [
            'parameters' => $parameters
        ]);
    }
     ...
     * @return mixed
     */
    private function countNewsByType($news)
    {
        $parameters['total'] = count($news);
        $enabledNews = [];
        $disabledNews = [];
        ...
        $parameters['enabledNews'] = count($enabledNews);
        $parameters['disabledNews'] = count($disabledNews);
     ...
     * @return mixed
     */
    private function countTeasersByType($teasers)
    {
        $parameters['total'] = count($teasers);
        $enabledTeasers = [];
        $disabledTeasers = [];
        ...
        foreach ($teasers as $item) {
            ...
            if ($item->getIsActive()) {
                ...
                $disabledTeasers[] = $item;
            }
        }

        $parameters['enabledTeasers'] = count($enabledTeasers);
        $parameters['disabledTeasers'] = count($disabledTeasers);
