...
class TeasersRepository extends ServiceEntityRepository
{
    ...
    public function getCountTeasers(User $user, $filterSubGroups = [], $search = null)
    {
        $params = [
            'user' => $user->getId(),
            'is_deleted' => 0,
    ...
    public function getCountTeasersOwnECPM(User $user, int $impressions)
    {
        $query = $this->createQueryBuilder('teasers')
            ->select('count(teasers.id) as count')
            ...
            ->andWhere('teasers.user = :user')
            ->andWhere('teasers.is_deleted = :is_deleted')
            ->andWhere('topTeaser.impressions = :impressions')
            ->setParameters([
                'user' => $user,
                'is_deleted' => 0,
