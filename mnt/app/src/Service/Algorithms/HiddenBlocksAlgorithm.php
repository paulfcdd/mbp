...
class HiddenBlocksAlgorithm extends AlgorithmAbstract
{
    ...
    public function getNewsForTop(int $page = 1): Collection
    {
        ...
        if(!$cacheNews->isHit()){
            ...
            $cacheNews->set($this->hideBlocks($news));
    ...
    public function getTeaserForTop(int $page = 1): Collection
    {
        ...
        if(!$cacheTeasers->isHit()){
            ...
            $cacheTeasers->set($this->hideBlocks($teasers));
    ...
    public function getTeaserForNews(News $news, int $page = 1): Collection
    {
        ...
        if(!$cacheTeasers->isHit()){
            ...
            $stmt = $this->entityManager->getConnection()->prepare($sql);
            $stmt->execute();
            $teasers = $this->hideBlocks(new ArrayCollection($stmt->fetchAll()));

            $cacheTeasers->set($this->combineHighAndLowItems($teasers, $this->getRandomLowTeasersForNews($news, $categories)));
    ...
    {
        if(isset($this->hiddenBlocks[$this->pageType][$this->getTrafficType()])){
            foreach($this->hiddenBlocks[$this->pageType][$this->getTrafficType()] as $hiddenBlock) {
                if($items[$hiddenBlock]) unset($items[$hiddenBlock]);
            }
        }
